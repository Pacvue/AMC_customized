...
def calculate_zscore_and_filter(df: pd.DataFrame,
    ...
    for col_name in numeric_cols:
        ...
        if col_std is not None and col_std > 0:
            ...
            # 如果 std=0，说明该列缺失值较多或者都是相同值，无需过滤
            pass
...
def data_preparation(input_path: str,
    ...
    数据预处理函数，包含以下步骤：
    1. 读取 CSV 文件
    2. 将日期字段转为 datetime，并计算二者的时间差（单位：天）
    3. 缺失值统计与填充
    4. 对 customer_search_term 列进行 Label Encoding（转换为数值型变量）
    5. 删除不需要的列：user_id, user_id_type
    6. 对其他类别变量进行 One-Hot 编码（如 device_type、browser_family、operating_system）
    ...
    :param input_path: CSV 文件路径（单个文件）
    :param threshold: Z-Score 过滤阈值，默认 3.0
    ...
    # ========== 9) 去重操作 ==========
    # 示例中这里以 customer_search_term 和 time_diff_days 去重，可根据需要调整
    if "customer_search_term" in df.columns and "time_diff_days" in df.columns:
        df = df.drop_duplicates(subset=["customer_search_term", "time_diff_days"])

    return df

def cleanup_output_folder(folder: str, allowed_filename: str):
    """
    清理输出目录中除 allowed_filename 之外的所有 CSV 文件，
    确保 train.py 在加载数据时只会读到 data_preparation.py 的输出。
    """
    all_csv_files = glob.glob(os.path.join(folder, "*.csv"))
    for f in all_csv_files:
        if os.path.basename(f) != allowed_filename:
            os.remove(f)
            print(f"Removed file: {f}")

if __name__ == "__main__":
    # ========== 读取并预处理数据 ==========
    # 请将 input_csv 替换为你要预处理的源文件路径
    input_csv = "/home/ec2-user/sylvia/HighPotentialCustomers/ml/input/data/train/sandbox_query_results_008da601-3002-45dd-867e-7c1c6fb01415_last90days.csv"
    df_clean = data_preparation(input_csv, threshold=3.0)

    # ========== 输出目录设置 ==========
    # train.py 的输入目录由环境变量 SM_CHANNEL_TRAIN 指定，默认路径如下：
    output_folder = os.environ.get("SM_CHANNEL_TRAIN", 
                                   "/home/ec2-user/sylvia/HighPotentialCustomers/ml/input/data/train/")
    if not os.path.exists(output_folder):
        os.makedirs(output_folder)

    # 指定 data_preparation.py 输出的文件名
    output_filename = "prepared_data.csv"
    output_file = os.path.join(output_folder, output_filename)

    # 清理输出目录中除本次输出文件之外的其他 CSV 文件
    cleanup_output_folder(output_folder, output_filename)

    # 写入处理后的数据到指定目录
    df_clean.to_csv(output_file, index=False)
    print(f"Prepared data is written to: {output_file}")
